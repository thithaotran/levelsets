...
def get_tape(ds, lss_phs1):
    with tf.GradientTape(persistent=True) as tape:
        ...
        for x, y in dataset_train:
            train_loss += tf.math.reduce_sum(tf.keras.losses.get(model.loss)(y, model(x)))
...
for exp_idx in range(cfg.EXP_INDEX_START, cfg.EXP_INDEX_START + cfg.NUM_EXP_TO_RUN):
    ...
    dataset_train = dataset_train.take(1000).cache().batch(32) # 1000 random examples
    ...
    # model.evaluate(dataset_train, verbose=False), model.evaluate(dataset_test, verbose=False)
